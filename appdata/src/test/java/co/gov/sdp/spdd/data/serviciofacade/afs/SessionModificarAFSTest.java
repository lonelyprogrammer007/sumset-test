...
public class SessionModificarAFSTest {

	@TestConfiguration
	static class SessionModificarAFSTestContextConfiguration {
		@Bean
		public SessionModificarAFS sessionModificarAFS() {
			return new SessionModificarAFS();
		}
	}

	@Autowired
	SessionModificarAFS sessionModifcarAFS;
	...
	@Test
	public void testModificarProyectoInstrumento() {

		PotProyectoInstrumentoDTO peticion=new PotProyectoInstrumentoDTO();
		peticion.setIdProyectoInstrumento(1L);
		PotProyectoInstrumento entity=new PotProyectoInstrumento();
		when(potProyectoInstrumentoServiceRepo.obtener(peticion.getIdProyectoInstrumento())).thenReturn(null);
	
		assertThat(sessionModifcarAFS.modificarProyectoInstrumento(peticion).getCodigoRespuesta()).isEqualTo(408);
        when(potProyectoInstrumentoMapper.potProyectoInstrumentoDTOToEntity(peticion)).thenReturn(entity);
		when(potProyectoInstrumentoServiceRepo.obtener(peticion.getIdProyectoInstrumento())).thenReturn(new PotProyectoInstrumento());
        when(potProyectoInstrumentoServiceRepo.guardar(entity)).thenReturn(entity);
        when(potProyectoInstrumentoMapper.potProyectoInstrumentoEntityToDTO(entity)).thenReturn(new PotProyectoInstrumentoDTO());
        assertThat(sessionModifcarAFS.modificarProyectoInstrumento(peticion).getCodigoRespuesta()).isEqualTo(200);
